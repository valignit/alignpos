...
class Invoice():
        ...
        while True:
            ...
            if event in ('F4:115', 'F4', 'Submit'):
                if len(self.__ui.items_list) > 0:
                    ...
                    self.upload_process()
                    self.download_process()                
                    if self.__ui.final_invoice_number:               
                        self.print_invoice()
                        self.clear_ui()
                        self.__actual_items_list = []                                
                    continue
    ...
    def payment(self):
        ...
        if not payment.output_param:
            return
        self.__ui.final_invoice_number = payment.output_param['final_invoice_number']
    ...
    def initialize_header_pane(self):
        self.__ui.draft_invoice_number = ''
        self.__ui.final_invoice_number = ''
    ...
    def goto_this_row(self, invoice_number):
        if invoice_number:
            if self.__menu_opt == 'operation':
                filter = "draft_invoice_number = '{}' and terminal_id = {}"
            else:
                filter = "final_invoice_number = '{}' and terminal_id = {}"
    ...
    def goto_first_row(self):
        if self.__menu_opt == 'operation':
            filter = "final_invoice_number is null and terminal_id = {}"
        else:
            filter = "final_invoice_number is not null and terminal_id = {}"
    ...
    def goto_previous_row(self): 
        ...
        if self.__menu_opt == 'operation':            
            if self.__ui.draft_invoice_number:
                ...
                filter = "name < '{}' and final_invoice_number is null and terminal_id = {}"
            ...
            if self.__ui.final_invoice_number:
                ...
                filter = "final_invoice_number < '{}' and final_invoice_number is not null and terminal_id = {}"
    ...
    def move_db_invoice_item_to_ui_detail_pane(self, db_invoice_item_row):
        self.__ui.item_code = db_invoice_item_row.item_code
        
        db_item_row = self.__db_item_table.get_row(db_invoice_item_row.item_code)
        if db_item_row:
            self.__ui.item_barcode = db_item_row.barcode
            self.__ui.item_name = db_item_row.item_name
            self.__ui.uom = db_item_row.uom
    ...
    def insert_invoice(self):
        ...
        db_invoice_row.draft_invoice_number = self.__ui.draft_invoice_number
        db_invoice_row.customer_id = customer_number
        ...
        for idx in range(len(self.__ui.items_list)): 
            ...
            db_invoice_item_row.invoice_item_id = self.__ui.draft_invoice_number + f"{idx:04d}"
            db_invoice_item_row.draft_invoice_number = self.__ui.draft_invoice_number
            db_invoice_item_row.item_code = self.__ui.item_code
    ...
    def update_invoice(self):
        ...
        if not db_invoice_row:
            return
        #db_invoice_row.posting_date = self.__ui.current_date    
        db_invoice_row.customer_id = self.__ui.customer_number
        db_invoice_row.total_amount = self.__ui.total_amount
        db_invoice_row.net_amount = self.__ui.net_amount
        db_invoice_row.invoice_amount = self.__ui.invoice_amount
        db_invoice_row.cgst_tax_amount = self.__ui.total_cgst_amount
    ...
    def print_invoice(self):
        ...
        #print('print:', self.__menu_opt, self.__ui.final_invoice_number)
        if self.__menu_opt == 'operation':
            if self.__ui.final_invoice_number:
                title = 'TAX INVOICE'
                invoice_number = self.__ui.final_invoice_number
...
where tabInvoice.customer_id = tabCustomer.name and tabInvoice.final_invoice_number is null'
            self.__order_by = ''
        else:
            self.__base_query = 'select tabInvoice.final_invoice_number, \
tabInvoice.total_amount, \
(tabInvoice.cgst_tax_amount + tabInvoice.sgst_tax_amount) as tax_amount, \
(tabInvoice.total_amount + tabInvoice.cgst_tax_amount + tabInvoice.sgst_tax_amount) as net_amount,\
ifnull(tabInvoice.discount_amount,0) as discount_amount,\
...
tabCustomer.mobile_number \
from tabInvoice, tabCustomer \
where tabInvoice.customer_id = tabCustomer.name and tabInvoice.final_invoice_number is not null'
            self.__order_by = ' order by tabInvoice.final_invoice_number'
                ...
                if self.__ui.invoice_number_search:
                    if not self.__ui.invoice_number_search == '':
                        if self.__menu_opt == 'operation':
                            ...
                            this_query = ' and tabInvoice.final_invoice_number = "' + self.__ui.invoice_number_search + '"'
...
class Payment:
                ...
                if float(self.__ui.balance_amount) == 0:
                    if float(self.__ui.other_payment_amount) > 0 and self.__ui.other_payment_reference == '':
                        ...
                        if self.validate_supervisor():
                            ...
                            msg = 'Invoice ' + str(self.__final_invoice_number) + ' generated'
                            Message('INFO', msg)                            
                            break
    ...
    def generate_invoice_number(self):
        db_query = DbQuery(self.__db_conn, 'SELECT nextval("FINAL_INVOICE_NUMBER")')
        for db_row in db_query.result:
            invoice_number = db_row[0]
        # Branch id embedded in Invoice number
        
        branch_id_str = '-'+ self.__branch_id + '-'
        self.__final_invoice_number = invoice_number.replace('-', branch_id_str)
        

    def set_output_parameters(self):
        self.__output_param['final_invoice_number'] = self.__final_invoice_number
    ...
    def update_database(self):
        ...
        db_invoice_row.final_invoice_number = self.__final_invoice_number
        db_invoice_row.customer_id = self.__ui.customer_number
        ...
        if db_exchange_row:
            db_exchange_row.invoice_number = self.__final_invoice_number
