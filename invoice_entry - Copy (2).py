...
def initialize_summary_pane():
    ...
    ui_summary_pane.total_qty = 0.00
...
def initialize_action_pane(context):
    if context == 'INVOICE':
        ui_window.Element('F1').update(text='Quick Items\nF1')
        ui_window.Element('F2').update(text='New Invoice\nF2')
        ui_window.Element('F3').update(text='Find Invoice\nF3', button_color = config["ui_function_button_color"], disabled = False)
        ui_window.Element('F4').update(text='Print Invoice\nF4', button_color = config["ui_function_button_color"], disabled = False)
        ui_window.Element('F5').update(text='Payment\nF5', button_color = config["ui_function_button_color"], disabled = False)
    else:
        ui_window.Element('F1').update(text='Quick Items\nF1')
        ui_window.Element('F2').update(text='Change Quantity\nF2')
        ui_window.Element('F3').update(text='', button_color='gray99', disabled = True)
        ui_window.Element('F4').update(text='', button_color='gray99', disabled = True)
        ui_window.Element('F5').update(text='', button_color='gray99', disabled = True)
...
def sum_item_list():
    ...
    ui_summary_pane.invoice_amount = net_amount - float(ui_summary_pane.discount_amount)
...
def open_message_popup(message):
    ...
    ui_popup = sg.Window('Confirm', [[sg.Text(message)],      
                     [sg.Button('Ok - F12', key='_MESSAGE_OK_'), sg.Button('Cancel - Esc', key='_MESSAGE_CANCEL_')]], keep_on_top=True, modal=True, return_keyboard_events=True, finalize=True)    
...
def main():
    ...
    while True:
        ...
        if event in ('Delete:46', 'Delete', 'DEL'):
            if prev_event == '_ITEMS_LIST_':
                ...
                if len(ui_detail_pane.items_list) > 0 and ui_header_pane.invoice_number == '':
                    #confirm_delete = sg.popup_yes_no('Invoice will be Deleted',keep_on_top = True, return_keyboard_events=True)
