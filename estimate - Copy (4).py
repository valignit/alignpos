...
class Estimate():
    ...
    def goto_first_row(estimate):
        db_estimate_row = estimate.db_estimate_table.first('')
        if db_estimate_row:
            estimate.clear_ui()    
            estimate.show_ui(db_estimate_row)    

    def goto_previous_row(estimate):
        if estimate.ui_header_pane.estimate_number:
            name = estimate.ui_header_pane.estimate_number
            filter = "name < '{}'"
            db_estimate_row = estimate.db_estimate_table.last(filter.format(name))
            if db_estimate_row:
                estimate.clear_ui()    
                estimate.show_ui(db_estimate_row)

    def goto_next_row(estimate):
        if estimate.ui_header_pane.estimate_number:
            name = estimate.ui_header_pane.estimate_number
            filter = "name > '{}'"
            db_estimate_row = estimate.db_estimate_table.first(filter.format(name))
            if db_estimate_row:
                estimate.clear_ui()    
                estimate.show_ui(db_estimate_row)
