...
# Description: Send the latest Item details created/updated after the last API call
# Version: 1.0
...
for ws_erp_row_item in ws_erp_resp_json["items"]:
    ...
    try:
        ...
        db_pos_conn.commit()
        print_log(f"Updated Item: {item_code, item_sync_date_time}")
    except mariadb.Error as db_err:
        print_log(f"POS database error: {db_err}")
        db_pos_conn.rollback()
        sys.exit(1)
    last_sync_date_time = item_sync_date_time
